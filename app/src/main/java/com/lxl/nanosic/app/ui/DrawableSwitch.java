...
public class DrawableSwitch extends View {
    ...
    private void init(AttributeSet attrs, Context context)
    {
        ...
        this.setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            {
                switchFocused(focused);
            }
        });
    }

    /**
     * 开关被选中时修改背景色，此处需要重绘界面
     */
    ...
    protected void changeStatus()
    {
        ...
        L.d("Encrypt isSwitchOn : " + isSwitchOn);

        // 保存当前的状态
        Utils.savePreferences(mContext,"encryption_switch", String.valueOf(isSwitchOn));
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        { // 打开时的状态
            ...
            float circleBaseline = radius + circleFontHeight / 2 - circleFontMetrics.bottom; // 计算文字绘制时的baseline
            canvas.drawText(circleText, switchWidth + radius - circleTextWidth/2, circleBaseline, paint); // 绘制文字，文字绘制的初始x坐标为右圆的圆心x坐标减去文字的宽度的一半
        ...
        { // 关闭时的状态
            ...
            // 再画右边的圆
            canvas.drawCircle(radius + switchWidth, radius, radius, paint);
            ...
            float circleFontHeight = circleFontMetrics.bottom - circleFontMetrics.top;
            float circleBaseline = radius + circleFontHeight / 2 - circleFontMetrics.bottom; // 计算文字绘制时的baseline
            canvas.drawText(circleText, radius - circleTextWidth/2, circleBaseline, paint); // 绘制文字，文字绘制的初始x坐标为右圆的圆心x坐标减去文字的宽度的一半
            ...
            float fontHeight = fontMetrics.bottom - fontMetrics.top;
            float baseline = radius + fontHeight / 2 - fontMetrics.bottom; // 计算文字绘制时的baseline
            canvas.drawText(text, switchWidth + radius - textWidth, baseline, paint); // 绘制文字，文字绘制的初始x坐标为右圆的圆心x坐标减去文字的宽度
        }
    }
